"""Load a model structure from RDF data in PRObs/``sphinx_probs_rdf`` form.

The RDF is queried for the processes belonging to a model (``probs:hasProcess``),
their recipes, and the objects those recipes reference. Which objects have markets
is read from the same data (``probs:hasMarketForObject``).

See :mod:`flowprog.io.system_definitions` to load the same information directly from
the ``system:process``/``system:object`` directive sources, without a Sphinx build.
"""
...
def load_structure(rdfox, model_uri, **kwargs) -> tuple[ModelStructure, dict]:
    """Load the :class:`~flowprog.model_structure.ModelStructure` for `model_uri`.

    :param rdfox: Something with a `query` method, such as an rdflib `Graph` or a
        `rdfox_runner` `RDFoxEndpoint`.
    :param model_uri: URI of the model to load, whose ``probs:hasProcess`` and
        ``probs:hasMarketForObject`` statements define what is in the model.
...
#: Previous name of :func:`load_structure`.
query_model_from_endpoint = load_structure
